...
function restoreProductStock(product: Product, items: StoreOrder["items"]): Product {
  ...
  if (product.variantStock && Object.keys(product.variantStock).length) {
    ...
    return {
      ...
      stock: totalStock({ ...product, variantStock: variantStockMap }),
    };
  }
  ...
  if (saved.order.paymentMethod === "card" && saved.order.total > 0) {
    if (!noonPaymentsConfigured()) {
      ...
      return NextResponse.json(
        {
          error:
            "Card checkout is not available right now. Please try again or choose cash on delivery.",
        },
        { status: 503 },
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Secure card checkout could not be opened. Please try again or choose cash on delivery.",
        },
        { status: 502 },
      );
    }
